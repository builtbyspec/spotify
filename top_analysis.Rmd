...
```
```{r warning=FALSE}
ggplot(data = lil_peep_time, aes(Date, stream)) + geom_line() + geom_path(na.rm = TRUE) + 
    geom_vline(data = lil_peep, aes(xintercept = Date), alpha = 0.4) + 
    geom_text(data = lil_peep, mapping = aes(label = Event, y = 0), angle = -10, hjust = 1)
  
```
```{r}
daily_top %>%
  mutate(Date = as.factor(Date)) %>%
  filter(Artist == "Linkin Park", Region == "us") %>%
  complete(Date) %>%
  group_by(Date) %>%
  summarize(stream = mean(Streams)) %>%
  mutate(Date = ymd(Date)) %>%
  ggplot(aes(Date, stream)) + geom_line()
```
```{r}
...
```{r warning=FALSE}
ggplot(data = xxx, aes(Date, stream)) + geom_line() + geom_path(na.rm = TRUE) + 
    geom_vline(data = xxx_events, aes(xintercept = Date), alpha = 0.4) + 
    geom_text(data = xxx_events, mapping = aes(label = Event, y = 0), angle = -10, hjust = 1,alpha = 0.9)
